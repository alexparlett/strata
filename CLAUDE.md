...
src/apps/project/                the project window (Valin-shaped)
  ...
  state/                         per-window state (Radio): channel, hooks, session
                                 ...
                                 one serial loop over both channels, asks before notices so a
                                 settle can never overtake the dispatch that minted its sequence
                                 number. It never waits for a query — the run is the directory's,
                                 on the engine — so all it does is check the agent holds the
                                 session, record what ran, and record what it came to. Holds the
                                 pure projections it answers with too (catalog from the store,
                                 never introspection), unit-tested with no renderer
  ...
  views/
    ...
    sidebar/
      ...
      agents/                    AA-03b — what each connected agent is doing: mod (pane + theme +
                                 ...
                                 has (the catalog's `SidebarRow`, the History drawer's card).
                                 **Only connected agents appear**, so no row wears a connected
                                 mark. A press opens a run's SQL in a **new** tab
                                 (`actions::open_sql`) — never the active one, which is the harm
                                 the pane exists to prevent — and there is no double-press to
                                 run. That press is the *only* way an agent's work reaches the
                                 tab strip
...
Feature specs: `AGENT_ACCESS_SPEC.md` (agent-driven access — the MCP host whose queries are real
runs in the agent's own query sessions, shown in the sidebar's Agents pane and promoted into a tab
by a press; the verified Tokio↔Freya bridge, the chat-pane forward design; dataflow in
