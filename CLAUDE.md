...
remaining workstreams (`.claude/tasks/`): statement completion (ED-11), the agent chat pane
...
session statements and `CREATE`/`DROP FUNCTION`; every intercepted kind has a real arm, what is
left is the completion offer that catches up with them (ED-11), and the agent stays read-only.
...
the open workstream tasks — statement completion (ED-11), the agent chat pane (AA-06) and the
