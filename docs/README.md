...
| [STATEMENTS_SPEC.md](STATEMENTS_SPEC.md) | The SQL statement surface: the classify → run/intercept/refuse router, internal tables (the `InternalTableStore` seam) and the writes over them, typed view DDL, typed `COPY`, the editor and agent policies, and what is still being lifted. |
