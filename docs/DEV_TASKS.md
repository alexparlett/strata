...
| S21 | Connections pane + remote object-store sources (v10) | ⬜ Todo | **Redesigned in v10** — Connections are a **project-scoped tool window on the left activity rail**, not a Settings category. Rail top group = **Catalog** \| **Connections** (`sidebarPane`; clicking the active pane collapses the sidebar, VS Code model). Pane lists saved object stores (provider icon · bucket · **status dot**: green *Connected* = env / anonymous / access-key-with-keys, amber *Needs credentials*); ⋮/right-click row-menu **Edit connection** / **Forget connection** (latter via remove-confirm); empty state + **Add**. **Add/Edit modal:** Bucket URL (Add-only, → provider label; read-only subtitle on Edit), **Authentication** segmented **Ambient (default provider chain) / Named profile / Anonymous**, **Region**, + a **profile picker** for the profile mode (names read from `~/.aws/config`); **no secret-entry fields — the app never takes or stores keys**; disabled until the URL resolves. Keyed by **scheme+authority (bucket)** in `connections` — the **same map** the Configure-table auth panel reads, so connecting a bucket flips referencing tables *Needs credentials → Connected* live. **The app never manages secrets.** Credentials resolve at **query time via the standard cloud provider chains** — AWS: env vars → shared `~/.aws/{config,credentials}` profiles → SSO → IAM role (`object_store::AmazonS3Builder::from_env()` + `aws-config` for profile resolution); GCS: application-default creds; Azure: env / managed identity. Keychain is **not** part of the AWS chain (an optional later integration, not baseline). A connection stores only **non-secret metadata** (bucket · provider · region · auth-mode · profile name). *(Diverges from the v10 design's Access-Key-ID/Secret form — that form should drop the secret fields.)* **Remote sources in Configure-table** (FEATURES §6): paths may be `s3://` / `gs://`·`gcs://` / `az://`·`abfs://` / `http(s)://` through one `ListingTableUrl` (globs/dirs/Hive work identically); remote panel names derived bucket/provider + live status; **public-bucket** toggle (→ `aws.SKIP_SIGNATURE true`, HTTP never needs creds); **one-table = one object store** guard (derived from the first path — mixing buckets/providers or local+remote is flagged + blocks Register, *"UNION in a view"*); a cloud path with no connection and not public blocks Register with an inline *connect-this-bucket* prompt. **Persistence:** connections carry **no secrets**, so the definition (bucket · provider · region · auth-mode · profile name) persists in the project's `.strata/` and reloads on open — hydrating the Connections pane + the Configure-table bucket matcher; saved on add / edit / forget. Bucket/provider is a shareable **def** (→ committed `project.json`); auth-mode + profile name are **per-machine** and may instead belong in the gitignored `session.json` (open Q). Either way **no key/secret ever touches disk via the app**. Design: v10 `Strata.dc.html` + FEATURES §6/§15b + CHANGELOG. *(The v9 Settings/Export/Configure window-frame unification is already shipped via the A3/A4 `Window`; Settings is now 100% global — Appearance / Data display / System / Keymap / Engine.)* |
