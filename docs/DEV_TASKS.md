...
| A7 | Split `ui/workspace.rs` into a module | ✅ Done | Broke the 683-line `workspace.rs` into `ui/workspace/` — `mod.rs` (`Workspace` shell + `CellView`), `tabs.rs`, `editor.rs`, `results.rs` (state switch + toolbar + pager + placeholders → future **R1**), `grid.rs` (`results_grid` / `render_cell` / `cell_dialog` → future **R3–R6**), `plan_view.rs` (`results_plan` / `plan_node_card` → future **S20**). Pure move (bodies unchanged; cross-module fns `pub(crate)`); `Workspace` stays the only `pub` item. |
