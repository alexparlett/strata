...
- **a task id** (`AA-04`, `P4-01`) — the diff above, plus that file under `.claude/tasks/`
  (a local backlog, where present) read as the acceptance contract.

### 2. Read enough to name the contract and the triggers

Read the changed files, and — where the local corpus exists — the `docs/reference/` entry for
each area the diff touches (routing table in `CLAUDE.local.md`). You are **not** forming an opinion here — you are
