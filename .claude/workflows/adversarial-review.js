...
const contract = a.contract || 'AGENTS.md, plus the matching docs/reference/ entry and any task file under .claude/tasks/ where those local files exist.'
...
const VOTER_LENS = [
  ...
  'CONTRACT. Does the cited rule say what the claim says it says? Open AGENTS.md (and the full entry in docs/reference/, where that corpus exists) and read the actual wording. Refute any rule cited by vibe, any pre-existing behaviour the diff did not change, and any preference wearing a severity label.',
