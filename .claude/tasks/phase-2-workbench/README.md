...
- **Stub / not built:** the grid still renders `fixture()` (the settled `QueryPage` is matched but
  unused — P2-03); `running.rs` and `explain_plan.rs` are one-line placeholders (their *states* are
  reached for real now); the status bar shows only a dot + state label + selection; there's no
...
- **Plumbing: done (P2-01 + P2-02).** Editor → Run/Explain/Analyze press → engine facade →
  results is wired end-to-end via freya-query: the workbench holds the `request` slot
  (`use_state(|| None::<QuerySpec>)`, threaded as props — no runs store), the results pane derives
  Empty / Running / Grid / ExplainPlan / **Error** from `use_query`'s state, and a settled `Err`
  renders a real error body (`results/error.rs`). Remaining plumbing is per-surface: page/sort
  reads into the grid (P2-03), plan rendering (P2-05), cancel (P2-15/06).
