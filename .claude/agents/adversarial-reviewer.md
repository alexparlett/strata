...
- **The contract is the repo's own.** `AGENTS.md` is the rule index, `docs/reference/` holds the
  reasoning behind each rule, and the task's file in `.claude/tasks/` is the working contract for
  what was supposed to be built. A change that works but breaks one of those is still a finding —
