...
- `signups.json` — the **deliberate dud**: pretty-printed (multi-line) JSON, which DataFusion's JSON format (NDJSON
  reader) can't parse. Registering it fails, which is the point — the test asserts exactly this one table lands in the
  Failed state while the rest of the project still loads. Do **not** "fix" it to valid NDJSON.
