...
//! [`quote_ident`](crate::engine::quote_ident) is *fold-preserving*: it renders a name so that
//! DataFusion resolves it to [`fold_ident(name)`](crate::engine::fold_ident), which is the
...
use crate::engine::sql::context::{LITERAL_WORDS, OPERAND_EXPECTING};
use crate::engine::sql::lex::is_reserved_in_name_position;
