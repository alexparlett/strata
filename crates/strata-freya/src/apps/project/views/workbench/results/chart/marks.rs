...
use super::paint::{Cross, Dress, Frame, Hit, PlotArea, Plotted};
...
/// Paint `frame` onto `context`'s canvas, recording into `plotted` where each mark landed and
/// where the plot's own frame ended up.
pub fn draw(context: &mut CanvasContext, frame: &Frame, plotted: &Plotted) {
    let mut marks = Vec::new();
    let mut area_out = None;
    let size = context.size;
    if size.width < MIN_WIDTH || size.height < MIN_HEIGHT {
        // Too small to have drawn anything, so there is nothing to be over either.
        *plotted.borrow_mut() = (Vec::new(), None);
        return;
    }
    ...
    // Replaced wholesale, never appended: what the pointer can be over — and where the plot
    // frame the crosshair rides in sits — is exactly what this paint drew.
    *plotted.borrow_mut() = (marks, area_out);
}
