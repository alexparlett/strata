...
//! about each ([`Reg`]). **Not** an introspection query against DataFusion, which would be wrong
//! in both directions: result snapshots are registered as real tables (`__snap_*`) and would show
//! up, and a def whose registration *failed* has no engine presence at all yet is exactly the row
