...
strata_components! {
    ...
    // and the rule under the header, the header's title, the Problems list's text ramp — a group
    // header's glyph / name / tally, a row's message, and the clean state's copy — the in-list
    // hairline between Events rows, and a History row's run figures, line-count pill and hover
    // surface. The severity ramp is NOT here: error / warning / info / success are semantic and
    // come off the sheet wherever they appear.
    "drawer" => DrawerThemePreference {
        background, border_fill, label_color, group_icon_color, group_color, meta_color,
        stats_color, badge_border_fill, message_color, row_hover_fill, divider_fill, empty_color,
    },
