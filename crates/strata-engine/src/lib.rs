...
pub struct Engine {
    ...
    /// What this engine last answered for each def it was asked to register — see [`Ledger`].
    ledger: Ledger,
...
impl Engine {
    ...
    /// **Move to the next generation and record what it answered for the workspace def `name`**
    /// — called from every funnel that registers a table or a view, in place of the bare bump
    /// those funnels used to make.
    ///
    /// The two are one call because they are one fact: the answer and the generation it was
    /// answered at are what a caller waiting on *this* registration compares (see
    /// [`Registrations::answered_since`]), and a bump that left the ledger behind would stamp
    /// the next answer with a number that had already gone by.
    fn note_registration(&self, name: &str, status: RegStatus) {
        let generation = self.generation.bump();
        self.ledger.note(name, status, generation);
    }
    ...
    fn settle_effect(&self, effect: Option<&StoreEffect>) {
        ...
        match effect {
            StoreEffect::TableUpserted { def, .. } => {
                ...
                self.note_registration(&def.name, RegStatus::Ready);
            }
