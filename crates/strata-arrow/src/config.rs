...
//!   and value validation, and by `strata_engine` to apply the overrides.
//!
...
//! Overrides live in [`strata_core::config::Settings::engine`], a `name → value` map.
//!
...
use strata_core::util::{is_byte_size, is_duration, is_time_zone};
...
pub const ENGINE_KEYS: &[EngineKey] = &[
    ...
    EngineKey {
        key: "datafusion.catalog.newlines_in_values",
        default: "false",
        kind: Kind::Bool,
    ...
    EngineKey {
        ...
        kind: Kind::Int { min: 1 },
        desc:
            "Rows per in-memory batch. Lower under tight memory; raise for vectorized throughput.",
    },
    EngineKey {
        key: "datafusion.execution.target_partitions",
        default: "0",
        kind: Kind::Int { min: 0 },
        desc: "Parallelism for execution. 0 = one partition per CPU core; 1 for tiny data.",
    },
    EngineKey {
        key: "datafusion.execution.time_zone",
        default: "+00:00",
        kind: Kind::TimeZone,
        desc: "Session time zone used by now() and timestamp functions.",
    },
    EngineKey {
        key: "datafusion.execution.coalesce_batches",
        default: "true",
        kind: Kind::Bool,
        desc: "Coalesce small batches between operators into larger ones.",
    },
    EngineKey {
        key: "datafusion.execution.collect_statistics",
        default: "true",
        kind: Kind::Bool,
        desc: "Collect statistics when a table is first created.",
    },
    EngineKey {
        key: "datafusion.execution.planning_concurrency",
        default: "0",
        kind: Kind::Int { min: 0 },
        desc: "Fan-out during physical planning. 0 = number of CPU cores.",
    },
    EngineKey {
        key: "datafusion.execution.spill_compression",
        default: "uncompressed",
        kind: Kind::Enum(&["uncompressed", "lz4_frame", "zstd"]),
        desc: "Codec for spill files: uncompressed, lz4_frame, or zstd.",
    },
    EngineKey {
        key: "datafusion.execution.sort_spill_reservation_bytes",
        default: "10485760",
        kind: Kind::Int { min: 0 },
        desc: "Memory reserved for each spillable sort's in-memory merge.",
    },
    EngineKey {
        key: "datafusion.execution.sort_in_place_threshold_bytes",
        default: "1048576",
        kind: Kind::Int { min: 0 },
        desc: "Below this size, sort in a single RecordBatch rather than merging.",
    },
    EngineKey {
        key: "datafusion.execution.meta_fetch_concurrency",
        default: "32",
        kind: Kind::Int { min: 1 },
        desc: "Files read in parallel when inferring schema and statistics.",
    },
    EngineKey {
        key: "datafusion.execution.enable_recursive_ctes",
        default: "true",
        kind: Kind::Bool,
        desc: "Support recursive common table expressions.",
    },
    EngineKey {
        key: "datafusion.execution.keep_partition_by_columns",
        default: "false",
        kind: Kind::Bool,
        desc: "Keep partition_by columns in the output RecordBatches.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.pruning",
        default: "true",
        kind: Kind::Bool,
        desc: "Skip row groups using predicate + min/max metadata.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.enable_page_index",
        default: "true",
        kind: Kind::Bool,
        desc: "Use the Parquet Page Index to reduce I/O and rows decoded.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.pushdown_filters",
        default: "false",
        kind: Kind::Bool,
        desc: "Apply filters during Parquet decoding (late materialization).",
    },
    EngineKey {
        key: "datafusion.execution.parquet.reorder_filters",
        default: "false",
        kind: Kind::Bool,
        desc: "Reorder pushed-down filters heuristically to cut evaluation cost.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.metadata_size_hint",
        default: "524288",
        kind: Kind::Int { min: 0 },
        desc: "Bytes fetched optimistically for the Parquet footer + metadata.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.compression",
        default: "zstd(3)",
        kind: Kind::Text,
        desc: "(writing) Default codec: snappy, gzip(level), zstd(level), lz4, lz4_raw.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.max_row_group_size",
        default: "1048576",
        kind: Kind::Int { min: 1 },
        desc: "(writing) Target max rows per row group.",
    },
    EngineKey {
        key: "datafusion.execution.parquet.statistics_enabled",
        default: "page",
    ...
    EngineKey {
        ...
        default: "true",
        kind: Kind::Bool,
        desc: "Prefer HashJoin over SortMergeJoin (faster, more memory).",
    },
    EngineKey {
        key: "datafusion.optimizer.repartition_joins",
        default: "true",
        kind: Kind::Bool,
        desc: "Repartition on join keys to run joins in parallel.",
    },
    EngineKey {
        key: "datafusion.optimizer.repartition_aggregations",
        default: "true",
        kind: Kind::Bool,
        desc: "Repartition on aggregate keys to run aggregates in parallel.",
    },
    EngineKey {
        key: "datafusion.optimizer.repartition_sorts",
        default: "true",
        kind: Kind::Bool,
        desc: "Sort per-partition then merge, rather than coalescing first.",
    },
    EngineKey {
        key: "datafusion.optimizer.repartition_file_scans",
        default: "true",
        kind: Kind::Bool,
        desc: "Repartition data-source partitions for maximum parallelism.",
    },
    EngineKey {
        key: "datafusion.optimizer.repartition_file_min_size",
        default: "10485760",
        kind: Kind::Int { min: 0 },
        desc: "Minimum total file size (bytes) to repartition a file scan.",
    },
    EngineKey {
        key: "datafusion.optimizer.enable_round_robin_repartition",
        default: "true",
        kind: Kind::Bool,
        desc: "Add round-robin repartitioning to use more CPU cores.",
    },
    EngineKey {
        key: "datafusion.optimizer.enable_topk_aggregation",
        default: "true",
        kind: Kind::Bool,
        desc: "Perform TopK during aggregations where possible.",
    },
    EngineKey {
        key: "datafusion.optimizer.enable_dynamic_filter_pushdown",
        default: "true",
        kind: Kind::Bool,
        desc: "Push operator-generated dynamic filters into the scan phase.",
    },
    EngineKey {
        key: "datafusion.optimizer.skip_failed_rules",
        default: "false",
        kind: Kind::Bool,
        desc: "Warn and continue when an optimizer rule errors, instead of failing.",
    },
    EngineKey {
        key: "datafusion.optimizer.max_passes",
        default: "3",
        kind: Kind::Int { min: 1 },
        desc: "How many times the optimizer re-runs over the plan.",
    },
    EngineKey {
        key: "datafusion.optimizer.default_filter_selectivity",
        default: "20",
        kind: Kind::Int { min: 0 },
        desc: "Default filter selectivity (0-100) when none can be determined.",
    },
    EngineKey {
        key: "datafusion.optimizer.hash_join_single_partition_threshold",
        default: "1048576",
        kind: Kind::Int { min: 0 },
    ...
    EngineKey {
        ...
        default: "false",
        kind: Kind::Bool,
        desc: "EXPLAIN prints only the logical plan.",
    },
    EngineKey {
        key: "datafusion.explain.physical_plan_only",
        default: "false",
        kind: Kind::Bool,
        desc: "EXPLAIN prints only the physical plan.",
    },
    EngineKey {
        key: "datafusion.explain.show_statistics",
        default: "false",
        kind: Kind::Bool,
        desc: "EXPLAIN prints operator statistics for physical plans.",
    },
    EngineKey {
        key: "datafusion.explain.show_sizes",
        default: "true",
        kind: Kind::Bool,
        desc: "EXPLAIN prints partition sizes.",
    },
    EngineKey {
        key: "datafusion.explain.format",
        default: "indent",
    ...
    EngineKey {
        ...
        default: "nulls_max",
        kind: Kind::Enum(&["nulls_max", "nulls_min", "nulls_first", "nulls_last"]),
        desc: "Default NULL ordering: nulls_max, nulls_min, nulls_first, nulls_last.",
    },
    EngineKey {
        key: "datafusion.sql_parser.enable_ident_normalization",
        default: "true",
        kind: Kind::Bool,
        desc: "Lower-case unquoted identifiers.",
    },
    EngineKey {
        key: "datafusion.sql_parser.parse_float_as_decimal",
        default: "false",
        kind: Kind::Bool,
        desc: "Parse float literals as DECIMAL.",
    },
    EngineKey {
        key: "datafusion.sql_parser.support_varchar_with_length",
        default: "true",
        kind: Kind::Bool,
        desc: "Permit VARCHAR(n) lengths (ignored) instead of erroring.",
    },
    EngineKey {
        key: "datafusion.sql_parser.map_string_types_to_utf8view",
        default: "true",
        kind: Kind::Bool,
        desc: "Map string types to Utf8View during planning.",
    },
    EngineKey {
        key: "datafusion.sql_parser.recursion_limit",
        default: "50",
        kind: Kind::Int { min: 1 },
    ...
    EngineKey {
        ...
        default: "%Y-%m-%d",
        kind: Kind::Text,
        desc: "strftime pattern for Date columns.",
    },
    EngineKey {
        key: "datafusion.format.datetime_format",
        default: "%Y-%m-%dT%H:%M:%S%.f",
        kind: Kind::Text,
        desc: "strftime pattern for DateTime columns.",
    },
    EngineKey {
        key: "datafusion.format.timestamp_format",
        default: "%Y-%m-%dT%H:%M:%S%.f",
        kind: Kind::Text,
        desc: "strftime pattern for Timestamp columns.",
    },
    EngineKey {
        key: "datafusion.format.time_format",
        default: "%H:%M:%S%.f",
        kind: Kind::Text,
        desc: "strftime pattern for Time columns.",
    },
    EngineKey {
        key: "datafusion.format.duration_format",
        default: "pretty",
    ...
    EngineKey {
        ...
        kind: Kind::Bytes,
        desc:
            "Cap on execution memory before spilling. Suffixes K/M/G, e.g. 2G. Blank = unlimited.",
    },
    EngineKey {
    ...
    EngineKey {
        key: "datafusion.runtime.temp_directory",
        default: "",
        kind: Kind::Text,
        desc: "Path to the temporary spill directory.",
    },
    EngineKey {
        key: "datafusion.runtime.metadata_cache_limit",
        default: "50M",
        kind: Kind::Bytes,
...
/// (`strata_engine::sql::lex`), and the UI resolves it through [`effective`] to
...
/// schema our tables live in (this crate's `CATALOG` / `SCHEMA`).
///
...
/// identity, and the session overlay refuses them toward Settings (ED-08) because the grid
